...
export function LuckyIncensePage() {
	...
	const [incenseCount, setIncenseCount] = useState<2 | 3>(2); // เลือกธูป 2 หรือ 3 ดอก
	...
	const generateNumbers = (): string => {
		const digitCount = incenseCount; // ใช้จำนวนธูปที่เลือก
		let numbers = "";
	 ...
	 */
	const calculateEmitterPosition = (): {
		emitterX: number;
		emitterY: number;
	} | null => {
									...
									<AnimatePresence>
										{isBurning &&
											...
											(() => {
												...
												const emitterX =
													emitterPos?.emitterX ?? defaultEmitterX;
												const emitterY =
													emitterPos?.emitterY ?? defaultEmitterY;
												const smokeImages = [
													"/image/smoke.png",
													"/image/smoke2.png",
													"/image/smoke3.png",
												...
												const safeSmokeParticles = Array.isArray(smokeParticles)
													? smokeParticles
													: [];
												const firstHalfParticles = safeSmokeParticles.slice(
													0,
													Math.floor(safeSmokeParticles.length / 2)
												);
												...
												return firstHalfParticles.length > 0 ? (
													...
													>
														{firstHalfParticles.map((particle, index) => {
															const smokeImage =
																smokeImages[index % smokeImages.length];
															...
															const progressMultiplier =
																index < firstHalfParticles.length * 0.3
															...
															const effectiveMultiplier =
																progressMultiplier > 0 ? progressMultiplier : 1;
															// ใช้ opacity แบบคงที่เพื่อให้ควันแสดงผลแน่นอน
															const smokeOpacity =
																baseOpacity * 0.6 * effectiveMultiplier;
										...
										{isBurning &&
											...
											(() => {
												...
												const safeSmokeParticles = Array.isArray(smokeParticles)
													? smokeParticles
													: [];
												const secondHalfParticles = safeSmokeParticles.slice(
													Math.floor(safeSmokeParticles.length / 2)
												);
												const behindParticles = secondHalfParticles.filter(
													(p) => !p.isFront
												);
												...
												return behindParticles.length > 0 ? (
													...
													>
														{behindParticles.map((particle, index) => {
															...
															// ลด opacity สำหรับควันบางตัวเพื่อไม่ให้หนาเกินไป (ควันที่ index สูงจะจางกว่า)
															const baseOpacity =
																index < behindParticles.length * 0.5
																	? 0.6
																	: 0.4;
															const imageOpacity =
																index < behindParticles.length * 0.5
															...
															// ควันที่ index ต่ำจะ fade out เร็วกว่า
															const progressMultiplier =
																index < behindParticles.length * 0.3
																	? Math.max(0, 1 - burnProgress / 50) // ควันชุดแรก fade out เมื่อ burnProgress > 50%
																	: Math.max(0, 1 - burnProgress / 100); // ควันชุดหลัง fade out ช้ากว่า
												...
												(() => {
													...
													return (
														<>
															...
															<motion.div
																initial={{ top: "0%", opacity: 0.35 }}
																animate={{
																	top: burnTopPosition,
																	opacity: shouldFadeOut ? 0 : 0.35,
																}}
																transition={{
																	top: {
																		duration: 0.016,
									...
									<AnimatePresence>
										{isBurning &&
											...
											(() => {
												...
												const frontParticles = secondHalfParticles.filter(
													(p) => p.isFront
												);
															...
															>
																{frontParticles.map((particle, index) => {
																	...
																	const startX =
																		...
																		additionalXOffset;
																	const startY =
																		emitterY + particle.initialY - 300;
																	...
																	const baseOpacity =
																		index < frontParticles.length * 0.5
																			? 0.4
																			: 0.25;
																	const imageOpacity =
																		index < frontParticles.length * 0.5
																	...
																	// ควันที่ index ต่ำจะ fade out เร็วกว่า
																	const progressMultiplier =
																		index < frontParticles.length * 0.3
																			? Math.max(0, 1 - burnProgress / 50) // ควันชุดแรก fade out เมื่อ burnProgress > 50%
																			: Math.max(0, 1 - burnProgress / 100); // ควันชุดหลัง fade out ช้ากว่า
										...
										>
											<div className="text-center rounded-2xl bg-card p-6">
												...
												<motion.p
													...
													style={{
														textShadow:
															"0 2px 8px rgba(48, 36, 174, 0.25), 0 0 12px rgba(200, 108, 215, 0.15)",
													}}
												>
													{result}
												</motion.p>
						...
						<div className="space-y-4">
							...
							>
								<p className="text-muted-foreground text-xs leading-relaxed text-center">
									⚠️ เลขที่ได้จากการจุดธูปนี้สร้างขึ้นเพื่อความบันเทิงเท่านั้น
									ไม่รับประกันผลลอตเตอรี่ทุกกรณี กรุณาเล่นอย่างมีสติ
								</p>
							</motion.div>
						</div>
					</div>
